...
function savePort(switchId, portNum) {
  ...
  // ── Remove old reverse link if the device changed ──
  if (prevDevId && prevDevId !== devId) {
    const prevDev = p.devices.find(d => d.id === prevDevId);
    if (prevDev && prevDev.portAssignments) {
      const prevPeerPort = (sw.portPeerPort||{})[portNum];
      if (prevPeerPort && prevDev.portAssignments[prevPeerPort] === switchId) {
